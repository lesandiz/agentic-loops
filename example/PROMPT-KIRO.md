...
| File       | Relative Path        |
| ---------- | -------------------- |
| PLAN.md    | `.ralph/PLAN.md`     |
| Specs      | `.ralph/specs/`      |
| ADRs       | `.ralph/adrs/`       |
| Scratchpad | `.ralph/scratchpad/` |
...
12. **Architecture Decision Records (ADRs)**: Create an ADR when you encounter a blocker requiring deviation from established patterns (new dependencies, data flow changes, infrastructure components).
    ...
    > 1. Document the decision needed in `.ralph/adrs/`
...
15. **Scratchpad**: Use `.ralph/scratchpad/` directory for temporary files during your turn.
...
1. Check line count: `wc -l .ralph/PLAN.md`
2. If **above 300 lines**: STOP and archive completed phases before proceeding
   - Move completed phase details to `.ralph/archive/completed_phases.md`
